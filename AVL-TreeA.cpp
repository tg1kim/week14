#include <stdio.h>
#pragma warning(disable: 4326 4996 6001 6031 6385)
...
int TreeWidth(TreePtr pTree)
{
	int nWdth = 0;
	if (pTree) {
		int nlWdth = TreeWidth(pTree->lChild);
		int nrWdth = TreeWidth(pTree->rChild);
		nWdth = NodeWIDTH + nlWdth;
		nrWdth && (nWdth += nrWdth - 1);
	}
	return nWdth;
}
